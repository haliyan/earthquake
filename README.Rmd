...
You can install the development version of earthquake like so: Enter `install_github("haliyan/earthquake")` into the console. 
...
The object `eq_data` will now be saved into the environment and can be cleaned and visualized. 
...
```{r,echo=FALSE}
eq_clean<-eq_raw_cleaner(as.data.frame(eq_data))
```
